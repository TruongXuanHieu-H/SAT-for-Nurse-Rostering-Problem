...
NRPEncoderAdder::NRPEncoderAdder(SATSolver *sat_solver, VarHandler *var_handler) : NRPEncoder(sat_solver, var_handler)
{
    std::cout << "c [NRPEncoderAdder] Encoder initialized for " << number_of_nurses << " nurses over " << schedule_period << " days.\n";
}

NRPEncoderAdder::~NRPEncoderAdder()
{
    
}

void NRPEncoderAdder::update_var_handler(const Clauses& clauses)
{
...
void NRPEncoderAdder::update_var_handler(const Clause& clause)
{
    for (int lit : clause)
    {
...
void NRPEncoderAdder::encode_at_least_20_work_shifts_every_28_days()
{
    ...
    for (int i = 0; i < number_of_nurses; ++i)
    {
        for (int j = 0; j <= schedule_period - 28; ++j)
        {
            ...
            std::vector<int64_t> weights(28);
            std::fill(weights.begin(), weights.end(), 1);

            Clauses clauses;
            pb2cnf->encodeGeq(weights, literals, 20, clauses, var_handler->peek_next_var());
            for (Clause clause : clauses)
            {
...
void NRPEncoderAdder::encode_at_least_4_off_days_every_14_days()
{
    ...
    for (int i = 0; i < number_of_nurses; ++i)
    {
        for (int j = 0; j <= schedule_period - 14; ++j)
        {
            ...
            std::vector<int64_t> weights(14);
            std::fill(weights.begin(), weights.end(), 1);
            
            Clauses clauses;
            pb2cnf->encodeGeq(weights, literals, 4, clauses, var_handler->peek_next_var());
            for (Clause clause : clauses)
            {
                if (!clause.empty() && clause.back() == 0)
                {
...
void NRPEncoderAdder::encode_between_1_and_4_night_shifts_every_14_days()
{
    ...
    for (int i = 0; i < number_of_nurses; ++i)
    {
        for (int j = 0; j <= schedule_period - 14; ++j)
        {
            ...
            Clauses clauses;
            pb2cnf->encodeLeq(weights, literals, 4, clauses, var_handler->peek_next_var());
            for (Clause clause : clauses)
            {
                if (!clause.empty() && clause.back() == 0)
                {
...
void NRPEncoderAdder::encode_between_4_and_8_evening_shifts_every_14_days()
{
    ...
    for (int i = 0; i < number_of_nurses; ++i)
    {
        for (int j = 0; j <= schedule_period - 14; ++j)
        {
            ...
            Clauses clauses;
            pb2cnf->encodeLeq(weights, literals, 8, clauses, var_handler->peek_next_var());
            for (Clause clause : clauses)
            {
                if (!clause.empty() && clause.back() == 0)
                {
...
void NRPEncoderAdder::encode_night_shifts_cannot_appear_on_consecutive_days()
{
    std::vector<std::vector<int>> night_shift(number_of_nurses, std::vector<int>(schedule_period, 0));
    for (int i = 0; i < number_of_nurses; ++i)
    {
...
void NRPEncoderAdder::encode_between_2_and_4_evening_or_night_shifts_every_7_days()
{
    ...
    for (int i = 0; i < number_of_nurses; ++i)
    {
        for (int j = 0; j <= schedule_period - 7; ++j)
        {
            ...
            Clauses clauses;
            pb2cnf->encodeGeq(weights, literals, 2, clauses, var_handler->peek_next_var());
            for (Clause clause : clauses)
            {
                if (!clause.empty() && clause.back() == 0)
                {
...
void NRPEncoderAdder::encode_at_most_6_work_shifts_every_7_days()
{
    std::vector<std::vector<int>> not_off_day(number_of_nurses, std::vector<int>(schedule_period, 0));
    for (int i = 0; i < number_of_nurses; ++i)
    {
    ...
    for (int i = 0; i < number_of_nurses; ++i)
    {
        for (int j = 0; j <= schedule_period - 7; ++j)
        {
            ...
            std::vector<int64_t> weights(7);
            std::fill(weights.begin(), weights.end(), 1);

            Clauses clauses;
            pb2cnf->encodeLeq(weights, literals, 6, clauses, var_handler->peek_next_var());
            for (Clause clause : clauses)
            {
                if (!clause.empty() && clause.back() == 0)
                {
